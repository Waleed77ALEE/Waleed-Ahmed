...
import { PlatformLogo, detectPlatform } from './PlatformLogo';
import {
  Search,
  ShoppingBag,
  Sparkles,
...
export const DigitalServices: React.FC<DigitalServicesProps> = ({ onSelectService, whatsappNumber, onAddToCart, onBuyNow }) => {
  ...
  const filteredServices = useMemo(() => {
    return services.filter((service) => {
      ...
      // Platform check
      const { key: pKey } = detectPlatform(service.title, service.category, service.subCategory, service.id);
      const matchesPlatform =
        selectedPlatformFilter === 'All' || pKey === selectedPlatformFilter;
          ...
          <div className="mb-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {featuredServices.map((service) => {
                const { info: pInfo } = detectPlatform(service.title, service.category, service.subCategory, service.id);
                return (
                  <div
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="flex items-start justify-between gap-3 mb-4">
                        <div className="flex items-center gap-2">
                          <div className="p-2.5 rounded-xl bg-slate-950 border border-slate-800 text-cyan-400 group-hover:scale-110 transition-all shadow-md flex items-center justify-center shrink-0">
                            <PlatformLogo title={service.title} category={service.category} subCategory={service.subCategory} id={service.id} className="w-7 h-7" />
                          </div>
        ...
        {filteredServices.length === 0 ? (
          <div className="text-center py-16 bg-slate-900/40 rounded-2xl border border-slate-800">
            ...
            <p className="text-xs text-slate-400 mb-4">
              Try adjusting your search query or switching category/platform filters.
            </p>
            <button
              onClick={() => {
                setSearchQuery('');
            ...
            {filteredServices.map((service) => (
              ...
              >
                <div>
                  ...
                  <div className="flex items-start justify-between gap-3 mb-4">
                    <div className="flex items-center gap-2">
                      <div className="p-2.5 rounded-xl bg-slate-950 border border-slate-800 text-cyan-400 group-hover:scale-110 transition-all shadow-sm flex items-center justify-center shrink-0">
                        <PlatformLogo title={service.title} category={service.category} subCategory={service.subCategory} id={service.id} className="w-6 h-6" />
                      </div>
                      <PlatformLogo title={service.title} category={service.category} subCategory={service.subCategory} id={service.id} showBadgeName={true} />
                    </div>
